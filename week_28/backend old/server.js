        ...
        Students.find().then(data => {
            ...
            if (data.length === 0) {
                Students.insertMany(initialData)
                    .then(() => console.log("Initial Data is Added to the database")).catch((err) => console.log("Error: " + err))
            }
            else {
...
app.get('/api/students', (req, res) => {
    ...
    Students.find().then(data => res.status(200).json(data)).catch(err => res.status(403).json("Error: " + err))
...
app.get('/api/students/:email', (req, res) => {
    ...
    Students.findOne({ email: query }).then(student => {
        if (student) {
    ...
    }).catch(err => res.status(404).json({ message: "No Such person Found", error: err }))
})
...
app.post('/api/students', (req, res) => {
    ...
    if (req.body.name === "" || !req.body.name) {
        ...
        data.save().then(() => res.status(200).json({ status: "Success", message: "New person has been added to the database" }))
...
app.delete('/api/students/:email', (req, res) => {
    ...
    .then((student) => {
        Students.findByIdAndDelete(student.id).then(() => res.status(200).json({ status: "Success", message: "person has been removed from the database" }))
    })
